...
AutoTileConverter.generateFullAutoTileBitmap = function(name, source) {
  ...
  var tileWidth = arr ? Number(arr[1]) : 32;


  regEx = /th\[(\d+)\]/i;
  arr = regEx.exec(name);
  name = name.replace(regEx, '');
  var tileHeight = arr ? Number(arr[1]) : 32;
  ...
  var folderName = 'dev\\Conveter\\out';
...
Scene_Boot.prototype.initialize = function() {
  ...
  var fs = require('fs');
  var base = path.dirname(process.mainModule.filename);
  var filePath = base + '\\dev\\Conveter\\in';
  ...
  for (var i = 0; i < dirList.length; i++) {
    ...
    // Get Bitmap
    var bitmap = ImageManager.loadBitmap('\\dev\\Conveter\\in\\', filename, 0, false)
